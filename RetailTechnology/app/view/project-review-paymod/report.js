define([
        ...
        'dojo/text!app/view/project-review-paymod/table.html',
    ...
    return {
        render: function (options) {
            ...
            if (options.data) {
                console.log("processData:" + options.data);
                processData(options.data);
            } else {
                console.log("construction.loadData");
            ...
            function processData(data) {
                ...
                var ITPM;
                if (hash.indexOf("itpm/") > -1) {
                    ITPM = hash.substring(hash.indexOf("itpm/") + "itpm/".length);
                    if (ITPM.indexOf("/") > -1)
                        ITPM = ITPM.substring(0, ITPM.indexOf("/"));
                }
                var CAMLQuery;
                if (ITPM)
                    CAMLQuery = "<Query><Where><And><Contains><FieldRef Name ='VP6800_x0020_IT_x0020_PM'/><Value Type = 'Text'>" + ITPM + "</Value></Contains><Geq><FieldRef Name='VP6800_x0020_Go_x0020_Live' /><Value IncludeTimeValue='False' Type='DateTime'><Today /></Value></Geq></And></Where><OrderBy><FieldRef Name='VP6800_x0020_Go_x0020_Live' /></OrderBy></Query>";
                else
