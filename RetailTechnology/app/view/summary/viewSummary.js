...
|| key === 'ProjectStatus') {
        ...
        function updateCostTracker(store) {
            ...
            //ISC - Total Cost:
            try {
                var ISCCostTotal = Big(currReplace(store.FABCONCost))
                    .plus(Big(currReplace(store.IDTECHCost)));
                summary.find('#ISCTotalCost').text(formatMoney(ISCCostTotal));
                store.ISCTotalCost = ISCCostTotal;
            }
            catch (err) {
                console.log("ISC - Total Cost - Addition Error:" + err.message);
            }
