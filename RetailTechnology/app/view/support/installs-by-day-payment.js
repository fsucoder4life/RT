define([
    'dojo/text!app/view/support/installs-by-day-payment.html',
    'app/view/report',
    "dojo/number",
    ...
    return {
        render: function (options) {
            ...
            _.forEach(options.data, function (store) {
                ...
                else if (store.InstallType === "PAYMENT") {
                    store.InstallType = "PAYMENT";
                }
            ...
            //Set the title
            $('#sub-title').html("Installs By Day");
...
//            me.projectType.set('value', options.posSelection);
            ...
            //POS
            $('#total-pos').html(dNumber.format(options.pos.total, {places: 0, locale: 'en-us'}));
            $('#micros-pos').html(dNumber.format(options.pos.micros, {places: 0, locale: 'en-us'}));
            $('#infor-pos').html(dNumber.format(options.pos.infor, {places: 0, locale: 'en-us'}));

            //POPS
            $('#total-pops-stores').html(dNumber.format(options.pops.stores, {places: 0, locale: 'en-us'}));
            $('#total-pops-units').html(dNumber.format(options.pops.units, {places: 0, locale: 'en-us'}));

            //Table Header Date
            $('#installs-header').html(moment(options.start).format('dddd, MMMM Do') + ' through ' + moment(options.end).format('dddd, MMMM Do') + " - Total Installs: " + options.data.length);
