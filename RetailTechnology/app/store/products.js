define(['app/utility/sp-utility', 'app/brands/services/brandServices', 'app/brands/services/logHelper'], function(utility,brandServices,logHelper){
  ...
  logHelper.logDebug("products.js", "webUrl: " + webUrl);
  //Request fields mapping from internal names
  var mapping = {
    ows_ID: {mappedName: 'ProductId', objectType: "Text"},
    ...
    ows_Manufacturer_x0020_Part_x0020_Nu : {mappedName: 'PartNumber', objectType: 'Text'},
    ows_DefaultQuantityFieldSource: {mappedName: 'DefaultQuantityFieldSource', objectType: 'Text'},
  ...
  function loadData (callback, options) {
    ...
    if (query === '' && typeof options.ProductId !== 'undefined') {
      query = new CamlBuilder().Where().TextField('ID').EqualTo(options.ProductId);
  ...
  function changeFactory (key, store, callback) {
    return function (newValue, oldValue, enable, el) {
      changeValue(key, newValue, store['ProductId'], function (response) {
        //Update store
        store[key] = newValue;
