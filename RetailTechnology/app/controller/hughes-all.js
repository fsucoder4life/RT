/**
 * Report/Workflow tool for Andy to schedule HUGHES
 */
define(['app/view/hughes/hughes', 'app/store/combined', 'app/store/construction', "dojo/hash", "dojo/number"], function (view, combined, construction, hash, dNumber) {
    return {
        show: function (target, options, routeCheck) {
            ...
            if (moment(options.start, 'YYYY-MM-DD', true).isValid() === false) {
                options.start = moment().add({weeks: -1}).format('YYYY-MM-DD');
            }
            if (moment(options.end, 'YYYY-MM-DD', true).isValid() === false) {
                options.end = moment().add({months: 4}).format('YYYY-MM-DD');
            }
            ...
            function complete () {
                if (constructionComplete && combinedComplete) {
                    ...
                    var columns = [
                        {key: 'InstallType', title: 'Install Type', transform: function (value, store) {
                            if (value !== "Construction*") {
                                return value;
                            } else {
                                return store.ProjectType + '*';
                            }
                        }},
                        ...
                        {key: 'PrimaryContact', title: 'Has POC', transform: function (value, store, stores, index) {
                            if (value !== '' && store.PrimaryPhone !== '') {
                                return 'Yes'
                            } else {
                                return '';
                            }
                    ...
                    view.render({
                        ...
                        callback: function (view) {
                            ...
                            view.button.on('click', function () {
                                ...
                                var fragment = "hughes/start/" + encodeURIComponent(moment(view.start.get('value')).format('YYYY-MM-DD')) +
