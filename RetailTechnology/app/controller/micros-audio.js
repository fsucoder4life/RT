        ...
        show: function (target, routeCheck, options) {
            ...
            combinedStore.loadData({query: query}, function (stores) {
                ...
                _.forEach(stores, function (store, index) {
                    if (
                      store.AudioType.toUpperCase().indexOf('MICROS') !== -1 &&
                      (
                        store.ProjectType !== 'POS Conversion' ||
                        store.AudioGoLiveDate > moment().hours(0).minutes(0).seconds(0).toISOString() ||
                ...
                constructionStore.loadData({query: constructionQuery}, function (constructionStores) {
                    ...
                    //Define report title
                    var title = 'Micros Audio - Upcoming Projects';
