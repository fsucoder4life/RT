define(['app/view/report', 'app/store/combined', 'app/store/construction', 'app/store/combinedconstructionextend'], function (report, combined, construction, combinedconstructionextend) {
    return {
        show: function (target, routeCheck, options) {
            ...
            //use SPServices to get all upcoming Audio go-live:

            var upcomingProjects = [];

            var useList = 'Combined Schedule';
            getUpcomingProjects('Audio', 'Audio_x0020_Go_x0020_Live_x0020_', useList);
            getUpcomingProjects('POS', 'GO_x0020_LIVE_x0020_DATE', useList);
            getUpcomingProjects('POS-Oracle 5810 Server', 'Oracle_x0020_5810_x0020_Server_x0', useList);
            getUpcomingProjects('POPS', 'POPS_x0020_Delivery_x0020_Date', useList);

            
            
            _.forEach(upcomingProjects, function (store) {
                console.log("storeNum: " + store.storeNum + " projType: " + store.projType + " date: " + store.goLiveDate);

            });

            function getUpcomingProjects(projType, field, list)
            {
                ...
                $().SPServices({
                    ...
                    async: false,
                    listName: list,
                    CAMLViewFields: "<ViewFields><FieldRef Name='Title' /><FieldRef Name='" + field + "' /></ViewFields>",
                    CAMLQuery: "<Query><Where><Geq><FieldRef Name='" + field + "' /><Value IncludeTimeValue='False' Type='DateTime'><Today /></Value></Geq></Where></Query>",
                    completefunc: function (xData, Status) {
                        $(xData.responseXML).SPFilterNode("z:row").each(function () {
                            
                            var valueToPush = {};
                            
                            valueToPush.projType = projType;
                            if (projType === "POPS") {
                                if (moment($(this).attr("ows_" + field)).isValid()) {
                                    valueToPush.goLiveDate = moment($(this).attr("ows_" + field)).add(1, 'days');
                                    valueToPush.storeNum = $(this).attr("ows_Title");
                                    upcomingProjects.push(valueToPush);
                                }

                            }
                            else {
                                valueToPush.goLiveDate = $(this).attr("ows_" + field);
                                valueToPush.storeNum = $(this).attr("ows_Title");
                                upcomingProjects.push(valueToPush);
                            }
                        });
            ...
            var columns = [
                ...
                { key: 'VP6800ReviewSignOffs', title: 'REVIEW SIGN-OFFS', transform: 'date' },
                
                
                
                
                { key: 'VP6800GoLive', title: 'DAY OF GO-LIVE', transform: 'date' },
            ...
            //Define report title
            var title = 'Next Gen Pays - Upcoming Dates';
