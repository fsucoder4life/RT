    ...
    return {
        show: function (target, routeCheck, options) {
            ...
            $().SPServices({
                ...
                CAMLQuery: "<Query><Where><Geq><FieldRef Name='Oracle_x0020_Server_x0020_Upgrad2' /><Value IncludeTimeValue='False' Type='DateTime'><Today OffsetDays='-2' /></Value></Geq></Where><OrderBy><FieldRef Name='Oracle_x0020_Server_x0020_Upgrad2'  /></OrderBy></Query>",
                async: false,
                completefunc: function (xData, Status) {
                    ...
                    $(xData.responseXML).SPFilterNode("z:row").each(function () {
                        ...
                        store.OverNightProjectType = "Oracle Server Swap";
                        store.StoreNumber = storeNum;
                        store.ConstructionId = $(this).attr("ows_ID");

                        store.OverNightInstaller = $(this).attr("ows_Oracle_x0020_Server_x0020_Upgrad3");

                        //Server Swap - go-live date:
                        store.OverNightDate = $(this).attr("ows_Oracle_x0020_Server_x0020_Upgrad2");
                        if (!store.OverNightDate)
                            store.OverNightDate = "";
            ...
            $().SPServices({
                ...
                CAMLQuery: CSquery,
                CAMLViewFields: "<ViewFields><FieldRef Name='ID' /><FieldRef Name='Title' /><FieldRef Name='POS_x0020_Selection' /><FieldRef Name='Address_x0020_Full' /><FieldRef Name='Zipcode' /><FieldRef Name='Franchise_x0020_Group' /><FieldRef Name='Market_x0020_DMA_x0020_Name_x002' /><FieldRef Name='City' /><FieldRef Name='State_x0020_' /></ViewFields>",
                async: false,
                completefunc: function (xData, Status) {
                    $(xData.responseXML).SPFilterNode("z:row").each(function () {
                        ...
                        for (var i = 0; i < stores2.length; i++) {
                            if (stores2[i].StoreNumber == storeNum) {
                                ...
                                stores2[i].Pos = $(this).attr("ows_Store_x0020_Phone");
                                if (!stores2[i].Pos)
                                    stores2[i].Pos = "";
            ...
            var columns = [
                ...
                { key: 'Pos', title: 'POS' },
                { key: 'OverNightProjectType', title: 'Install Type' },
                { key: 'OverNightInstaller', title: 'Installer' },
                
                
                { key: 'OracleServerUpgradeGoLive', title: 'Server Swap Go Live', transform: 'date', editable: true },
                { key: 'OracleServerUpgradeGoLive2', title: 'Win 10 Upgrade Go Live<br />(sort ignored)', transform: 'date', editable: true }
