    ...
    return {
        show: function (target, routeCheck, options) {
            ...
            var query = new CamlBuilder().Where().All(
                ...
                // CamlBuilder.Expression().DateField('DT_x0020_POPS_x0020_Delivery_x00').NotEqualTo(''),
                // CamlBuilder.Expression().DateField('DT_x0020_POPS_x0020_Delivery_x00').IsNotNull()
            );
            ...
            //Define report title
            var title = 'DT POPS - Historical & Future Projects';
            ...
            //Define Filtering - also using this to standardize dates between construction and conversion stores
            var filter = function (arr) {
                ...
                _.forEach(arr, function (store, index) {
                    ...
                    //Use the POPS delivery date if there is no DT POPS delivery date and it's a construction project
                    if (store.ProjectType !== 'POS Conversion' && (store.DtPopsDeliveryDate === '' || typeof store.DtPopsDeliveryDate === 'undefined')){
                        store.DtPopsDeliveryDate = store.PopsDeliveryDate;
                    }
                    ...
                    if (store.ProjectType !== 'POS Conversion' && moment(store.DtPopsDeliveryDate).startOf('day').toISOString() <= installDate) {
                        ...
                        store.DtPopsInstallDate = store.InstallDate;
                        ...
                        console.log(store.StoreNumber, store.DtPopsInstallDate);
                    }
                });
